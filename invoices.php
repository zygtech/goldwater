...
?>
	<h1 style="float: left;">Invoices
	</h1>
	<span style="float: right;"><a href="invoice.php"><i class="fa fa-plus" aria-hidden="true"></i> Add an invoice</a></span><br /><br />
	<table class="data">
	<?php
		...
		?>
		<tr><th style="width: 20%;"><a href="invoices.php?sort=invoiceid<?php if ($_GET['sort']=='invoiceid' && $_GET['order']=='') echo '&order=DESC'; ?><?php if ($_GET['q']!='') echo '&q=' . $_GET['q']; ?>">INVOICE ID</a></th><th style="width: 30%;"><a href="invoices.php?sort=company<?php if ($_GET['sort']=='company' && $_GET['order']=='') echo '&order=DESC'; ?><?php if ($_GET['q']!='') echo '&q=' . $_GET['q']; ?>">CLIENT</a></th><th style="width: 15%;"><a href="invoices.php?sort=added<?php if ($_GET['sort']=='added' && $_GET['order']=='') echo '&order=DESC'; ?><?php if ($_GET['q']!='') echo '&q=' . $_GET['q']; ?>">BY</a></th><th style="width: 15%;"><a href="invoices.php?sort=creation<?php if ($_GET['sort']=='creation' && $_GET['order']=='') echo '&order=DESC'; ?><?php if ($_GET['q']!='') echo '&q=' . $_GET['q']; ?>">DATE</a></th><th style="width: 20%;"><a href="invoices.php?sort=total<?php if ($_GET['sort']=='total' && $_GET['order']=='') echo '&order=DESC'; ?><?php if ($_GET['q']!='') echo '&q=' . $_GET['q']; ?>">TOTAL</a></th><th></th></th><th></th><th></th></tr>
		<?php
		$p=1; $l=0;
		while ($row = mysqli_fetch_array($result)) {
			...
			if ($_GET['q']=='' || strpos(strtolower($row['id']), strtolower($_GET['q']))!==false || strpos(strtolower($row['company']), strtolower($_GET['q']))!==false || strpos(strtolower($row['fullname']), strtolower($_GET['q']))!==false || strpos(strtolower($row['creation']), strtolower($_GET['q']))!==false || strpos(strtolower($row['total']), strtolower($_GET['q']))!==false) {				
				...
				echo ' <a href="client.php?id=' . $row['client'] . '" style="background: none; color: #58585a;" title="Client edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td><td style="width: 15%;">' . $row['added'] . '</td><td style="width: 15%;">' . $row['creation'] . '</td><td style="width: 20%;">';
				if ($row['currency']=='USD') echo '$'; if ($row['currency']=='EUR') echo '€'; if ($row['currency']=='GBP') echo '£';
				echo number_format($row['total'],2,'.','');
				if ($row['currency']=='PLN') echo ' zł';
				echo '</td><td><a href="invoicepdf.php?id=' . $row['id'] . '&company=' . $_SESSION['company'] . '&c=' . md5($row['id'] . $_SESSION['company'] . 'IGW') . '" title="Invoice view"><i class="fa fa-search" aria-hidden="true"></i></a></td><td><a href="invoicesave.php?id=' . $row['id'] . '&company=' . $_SESSION['company'] . '&c=' . md5($row['id'] . $_SESSION['company'] . 'IGW') . '" title="Invoice save"><i class="fa fa-save" aria-hidden="true"></i></a></td><td><a href="invoice.php?id=' . $row['id'] . '" title="Invoice edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td><td><a class="confirm" href="invoicedel.php?id=' . $row['id'] . '" title="Invoice delete"><i class="fa fa-trash" aria-hidden="true"></i></a></td></tr>';
				$l++;
			}
		}
	...
	?></center>
<?php
	echo '<br /><a href="iexport.php?q=' . $_GET['q'] . '&sort=' . $_GET['sort'] . '&order=' . $_GET['order'] . '" target="_BLANK">Export to CSV</a>';
