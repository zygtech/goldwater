...
<?php
	...
	if (strip_tags($_POST['name'])!='' && $_SESSION['login']!='' && $_POST['id']=='' && $clientid!='')
		mysqli_query($link,'INSERT INTO `' . $_SESSION['company'] . '_quotes` VALUES (0,"' . strip_tags($_POST['name']) . '",' . $clientid . ',"' . strip_tags($clientinfo) . '","' . strip_tags($_POST['description']) . '",NOW(),"' . $_SESSION['login'] . '","' . strip_tags($_POST['quoteid']) . '");');
	elseif (strip_tags($_POST['name'])!='' && $_SESSION['login']!='' && $clientid!='')
		mysqli_query($link,'UPDATE `' . $_SESSION['company'] . '_quotes` SET name="' . strip_tags($_POST['name']) . '", client=' . $clientid . ', clientinfo="' . strip_tags($clientinfo) . '", description="' . strip_tags($_POST['description']) . '", quoteid="' . strip_tags($_POST['quoteid']) . '" WHERE id=' . $_POST['id'] . ';');
	mysqli_free_result($result);
	mysqli_close($link);
?>
<meta http-equiv="refresh" content="0;url=<?php echo $url; ?>/pl/quotes.php"> 
