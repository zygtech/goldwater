	...
	<?php
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if ($_GET['q']=='' || strpos(strtolower($row['id']), strtolower($_GET['q']))!==false || strpos(strtolower($row['name']), strtolower($_GET['q']))!==false || strpos(strtolower($row['category']), strtolower($_GET['q']))!==false || strpos(strtolower($row['price']), strtolower($_GET['q']))!==false) {				
				...
				echo '</td><td><a href="productpdf.php?id=' . $row['id'] . '" title="Product view"><i class="fa fa-search" aria-hidden="true"></i></a></td><td><a href="productsave.php?id=' . $row['id'] . '" title="Product save"><i class="fa fa-save" aria-hidden="true"></i></a></td><td>';
				if ($row['archive']==0) 
